...
def generate_exp_coeffs(nets, opt, mode='latent'):
    if mode=='latent':
        ...
        opt.ref_dirs = [os.path.join(opt.celeb, d) for d in ('reference/Nicholson/DECA', 'reference/Pacino/DECA', 'reference/DeNiro/DECA')]
    else:
        raise NotImplementedError
    opt.batch_size = 1
    num_exp_coeffs = 51

    loader_src = get_test_loader(os.path.join(opt.celeb, 'test/Pacino/DECA'), opt)
